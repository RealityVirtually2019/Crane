...
namespace Leap.Unity.Examples {
  ...
  [AddComponentMenu("")]
  public class TransformTool2 : MonoBehaviour {

    [Tooltip("The scene's InteractionManager, used to get InteractionControllers and "
           + "manage handle state.")]
    public InteractionManager interactionManager2;
    ...
    void Start() {
      if (interactionManager2 == null) {
        interactionManager2 = InteractionManager.instance;
      }
    ...
    void Update() {
      ...
      target.transform.position = new Vector3 (-0.5f, 1.25f, Mathf.Clamp(target.transform.position.z, OutLimit, InLimit));
      ThisHandle.transform.position = new Vector3 (-0.5f, 1.25f, Mathf.Clamp(ThisHandle.transform.position.z, 0.1f, InLimit));
////////
    ...
    private void updateHandles() {
      switch (_toolState) {
        case ToolState.Idle:
          ...
          foreach (var intController in interactionManager2.interactionControllers
                                                          .Query()
