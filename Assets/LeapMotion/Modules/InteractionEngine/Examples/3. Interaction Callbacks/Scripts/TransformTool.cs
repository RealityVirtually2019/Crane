...
namespace Leap.Unity.Examples {
  ...
  public class TransformTool : MonoBehaviour {
    ...
    public float OutLimit = 0.14f;
    public float InLimit = 0.5f;
    
    public GameObject slide;
    //PosRefTraslation
    
    public GameObject ThisHandle;

    bool MoveLimitCheck = false;

    private Vector3    _moveBuffer = Vector3.zero;
    private Quaternion _rotateBuffer = Quaternion.identity;//rotation
    ...
    public Vector3 clampPositionTarget = new Vector3(0, 1.25f, 0);
    void Update() {
      // Enable or disable handles based on hand proximity and tool state.
      updateHandles();
      // Debug.Log(target.position.z);

      clampPositionTarget.z = Mathf.Clamp(target.transform.position.z, OutLimit, InLimit);
      target.transform.position = clampPositionTarget;

      Vector3 clampPositionHandle = clampPositionTarget;
      clampPositionHandle.z = Mathf.Clamp(ThisHandle.transform.position.z, 0.1f, InLimit);
      ThisHandle.transform.position = clampPositionHandle;

      // target.transform.position = new Vector3 (0, 1.25f, Mathf.Clamp(target.transform.position.z, OutLimit, InLimit));
      // ThisHandle.transform.position = new Vector3 (0, 1.25f, Mathf.Clamp(ThisHandle.transform.position.z, 0.1f, InLimit));
////////
      if(target.transform.position.z == OutLimit)
      {
